...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp)
{
    ...
    if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET) {
        ...
        mCurrentFrame.mRawFrame = mImGray;
        mCurrentFrame.mRawFrameBGR = im;
    }
    else {
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        if(bOK)
        {
            ...
            mCurrentPose =  mCurrentFrame.mTcw;
//            [mpDepthRenderer SetCurrentCameraPose:mCurrentFrame.mTcw];
...
void Tracking::CreateNewKeyFrame()
{
    ...
    pKF->mFrameForDepth = mCurrentFrame.mRawFrame.clone();
    pKF->mFrameBGR = mCurrentFrame.mRawFrameBGR.clone();
...
void Tracking::UpdateLocalKeyFrames()
{
    ...
    int max=0, maxBaseline = 0;
    KeyFrame* pKFmax= static_cast<KeyFrame*>(NULL);
    KeyFrame* pDepthKFmax = static_cast<KeyFrame*>(NULL);
    ...
    for(map<KeyFrame*,int>::const_iterator it=keyframeCounter.begin(), itEnd=keyframeCounter.end(); it!=itEnd; it++)
    {
        ...
        if(it->second>max)
        {
//            printf("observed ratio:%d/%d\n", it->second, goodMP);
            max=it->second;
            pKFmax=pKF;
        }
        
        if (it->second / float(goodMP) > 0.75) {
            float baseline = norm(pKF->GetPose(), mCurrentFrame.mTcw);
            if (baseline > maxBaseline) { // find the longest baseline
                maxBaseline = it->second;
                pDepthKFmax = pKF;
            }
        }
    ...
    if (pDepthKFmax) {
        mpDepthReferenceKF = pDepthKFmax;
    }
}
