...
{
    if (!detector.get()) {
        detector = cv::BRISK::create(10);
    }
    if (!computor.get()) {
        computor = cv::BRISK::create(10);
    }
...
arma::mat BriskKeypointMatcher3D2D::MatchImagesByKeypoints(const vector<cv::KeyPoint> &referenceKeypoints, const vector<cv::KeyPoint> &inputKeypoints)
{
    ...
    {
        cv::Point2d refPoint( referenceKeypoints[i].pt.x, referenceKeypoints[i].pt.y );
        arma::rowvec intersectPoint;
        bool isIntersect = this->find3DPointOnMesh(refPoint, intersectPoint);
        ...
        {
            this->bary3DRefKeypoints(i, arma::span(0,5)) = intersectPoint;
            matches3D2D(i, arma::span(0,5)) = this->bary3DRefKeypoints.row(i);
...
arma::mat BriskKeypointMatcher3D2D::MatchImages3D2D(cv::Mat& inputImageGray)
{
