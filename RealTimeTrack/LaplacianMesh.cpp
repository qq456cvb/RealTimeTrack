...
void LaplacianMesh::SetVertexCoords(const mat& vtCoords)
{
    ...
    this->bigAP.reset();
    this->bigReglMat2D.reset();
    this->bigParamMat2D.reset();
    this->bigAP2D.reset();
}
...
void LaplacianMesh::computeRegMatrix()
{
    ...
    this->bigReglMat2D = kron( eye(2,2), regMat );
}
...
void LaplacianMesh::computeParamMatrix()
{
    ...
    this->bigParamMat2D = kron( eye(2,2), paramMat );
    
    mat ctrlPoints = this->GetVertexCoords().rows(this->GetCtrlPointIDs());
    if (Configuration::enableDeformTracking) {
        ctrlPoints.reshape(75, 1);
    } else {
        ctrlPoints.reshape(12, 1);
    }
    
    mat points = this->GetVertexCoords();
    points.reshape(points.n_elem, 1);
    cout << this->bigParamMat.n_rows << " " << this->bigParamMat.n_cols << endl;
    cout << arma::mean(arma::abs(this->bigParamMat * ctrlPoints - points)) << endl;
//    this->bigParamMat * this->GetVertexCoords().rows(this->GetCtrlPointIDs())
}
...
void LaplacianMesh::ComputeAPMatrices()
{
    ...
    this->bigAP2D = bigReglMat2D * bigParamMat2D;
}
