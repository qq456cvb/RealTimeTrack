...
private:
    ...
    SoftConstrOptimize softConstrOptimize;
...
public:
	...
	Reconstruction(const LaplacianMesh &pRefMesh, const Camera &pCamCamera, 
                    ...
                    softConstrOptimize (SoftConstrOptimize(pNCstrIters, refMesh))
	{
       // this->softConstrOptimize = SoftConstrOptimize(pNCstrIters, refMesh);
		// Compute (AP)' * (AP) to avoid recomputation
		const arma::mat& bigAP = refMesh.GetBigAP();
        const arma::mat& bigAP2D = refMesh.GetBigAP2D();
		this->APtAP = bigAP.t() * bigAP;
        this->APtAP2D = bigAP2D.t() * bigAP2D;
    ...
    void ReconstructPlanarUnconstrIter2D( const arma::mat& matches, LaplacianMesh& resMesh, arma::uvec& inlierMatchIdxs );
    
    void ReconstructPlanarUnconstrOnce( const arma::mat& matches, LaplacianMesh& resMesh );
    
    void ReconstructPlanarUnconstrIter( const arma::mat& matches, LaplacianMesh& resMesh, arma::uvec& inlierMatchIdxs, int tolerance );
    ...
    void ReconstructSoftConstr(const arma::vec& cInit, LaplacianMesh& resMesh);
    void ReconstructSoftConstr2D(const arma::vec& cInit, LaplacianMesh& resMesh);
private:
    ...
    void buildCorrespondenceMatrix2D( const mat& matches );
    ...
    void computeCurrentMatrices2D( const arma::uvec& matchIdxs, double wr );
    void computeCurrentBPwAP( const arma::uvec& matchIdxs, double wr );
    ...
    vec computeReprojectionErrors2D( const TriangleMesh& trigMesh, const mat& matchesInit, const uvec& currentMatchIdxs, bool );
    
    void updateMesh( LaplacianMesh& resMesh );
    ...
    void reconstructPlanarUnconstr2D( const arma::uvec& matchIdxs, double wr, LaplacianMesh& resMesh );
};
