...
namespace DBoW2 {
    ...
    public:
        ...
        bool loadFromTextFile_old(const std::string &filename);
        /**
    ...
    bool TemplatedVocabulary<TDescriptor,F>::loadFromTextFile(const std::string &filename)
    {
        ...
        while(!f.eof())
        {
            ...
            cv::Mat aa = cv::Mat(1, 32, CV_8U);
            for(int i = 0; i < 32; ++i)
            { 
                aa.at<uchar>(i) = (unsigned char)(d2[i+2]);
            }
            aa.copyTo(m_nodes[nid].descriptor);
            
            m_nodes[nid].weight = d2[34];
    ...
    template<class TDescriptor, class F>
    bool TemplatedVocabulary<TDescriptor,F>::loadFromTextFile_old(const std::string &filename)
    {
        ifstream f;
