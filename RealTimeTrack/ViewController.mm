...
- (void)viewDidLoad {
    ...
    imageChoose = new ImageChoose(*(refMesh), modelCamCamera);
    ...
    reconstruction      = new Reconstruction  ( *refMesh, realCamCamera, wrInit, radiusInit, nUnConstrIters );
...
//        cv::cvtColor( inputImgRGB, inputImgGray, cv::COLOR_BGR2GRAY );
        ...
        if (lostTrack) {
            ...
            int vote;
            vote = imageChoose->Vote(image, refKeypoints, inputKeypoints, ref3DPoints);
            //assert(ref3DPoints.n_cols == 6);
            timer.stop();
            ...
            if (vote > DETECT_THRES) {
                ...
                matchesRaw = keypointMatcher->MatchImagesByKeypoints(refKeypoints, inputKeypoints, ref3DPoints); // TODO: Make the input gray image
                //matchesRaw = keypointMatcher->MatchImages3D2D(inputImgGray);
