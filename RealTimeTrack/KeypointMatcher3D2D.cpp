...
camCamera(camCamera)
{
    ...
    this->referenceImgRGB = referenceImgRGB.clone();
    ...
    cv::cvtColor(referenceImgRGB, referenceImgGray, cv::COLOR_BGR2GRAY);
    ...
    //const cv::Size templateSize = cv::Size(templateWidth, templateHeight);
    const cv::Rect templateRect = cv::Rect(topLeft.x, topLeft.y, templateWidth, templateHeight);
    ...
    this->templateImageGray = cv::Mat(this->referenceImgGray, templateRect).clone();
...
arma::mat KeypointMatcher3D2D::MatchImages3D2D(const cv::Mat& inputImageRGB)
{
